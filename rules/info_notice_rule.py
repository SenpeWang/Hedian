...
class InfoNoticeRule(BaseRule):
    ...
    def _start_flow(self, timestamp: float, source: str) -> None:
        """启动信息通报流程.
        ...
        has_hand_raise = (0 <= (timestamp - self._last_hand_raise_ts) <= 5.0)
        ...
        logger.info(
            f"信息通报流程开始 flow_id={self._flow_id} @{timestamp:.1f}s 伴随举手={has_hand_raise} source={source}"
        )
    ...
    def _close_flow(
        ...
        logger.info(
            f"信息通报流程结束 flow_id={self._flow_id} @{timestamp:.1f}s source={source}")
    ...
    def _on_voice_intent(self, event: Dict[str, Any]) -> None:
        ...
        # 超时自动关闭
        if self._active and timestamp - self._flow_start_sec > 180.0:
        ...
        if key_moment in ["信息通报", "信息通告"]:
            logger.debug(f"信息通报: 收到信息通报语音事件 @{timestamp:.1f}s，开始流程")
            self._start_flow(timestamp, source="voice")
        elif key_moment in ["通报完毕", "通告完毕"]:
            if self._active:
                self._checklist["shout_finished"] = True
                logger.info(f"信息通报: 喊出'通报完毕' @{timestamp:.1f}s，流程结束")
                self._close_flow(timestamp, source="normal_end")
        elif key_moment == "收到":
            if self._active:
    ...
    def _on_gaze_status(self, event: Dict[str, Any]) -> None:
        ...
        if has_turned:
            ...
            logger.info(f"信息通报: 团队成员均予以关注 @{timestamp:.1f}s")
