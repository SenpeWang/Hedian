 ...
 * 运行时指标结构: 累计计数器 + 最新值快照.
 *
 * 每次快照刷新到 window.__playbackMetrics(devtools 排查, 不外发).
 */
...
declare global {
  interface Window {
    __playbackMetrics?: PlaybackMetrics
  }
}

/**
 * 指标存储: 计数器累加 + 快照写入 window, 供 devtools 在线排查.
 */
class MetricsStore {
  ...
  flush(): void {
    try {
      window.__playbackMetrics = {
        ...this.data,
        bufferLevel: { ...this.data.bufferLevel },
        queueDepth: { ...this.data.queueDepth }
      }
    } catch {
      /* SSR/隐私模式忽略 */
    }
  ...
  /**
   * 全量清零(重新推理时调用), 并立即刷新一次快照.
   */
  reset(): void {
    this.data.appendFail = 0
    this.data.quotaErrors = 0
    this.data.droppedSegments = 0
    this.data.reportCopiesReleased = 0
    this.data.reconnects = 0
    this.data.seekCount = 0
    this.data.seekFailCount = 0
    this.data.clock = 0
    this.data.rate = 1.0
    this.data.slaveDrift = 0
